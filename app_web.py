...
if  choix_mode == "⌨️ Recherche manuelle":
    ...
    # Injection du conteneur de caméra et de la logique de capture instantanée
    scanner_html = """
    <div style="text-align: center; font-family: sans-serif;">
        <div id="interactive-reader" style="width: 100%; max-width: 480px; margin: auto; border: 2px solid #003366; border-radius: 8px; overflow: hidden; background-color: #000;"></div>
        <input type="hidden" id="result-bridge" value="">
    </div>
    <script src="https://unpkg.com"></script>
    <script>
        // Demande explicitement l'autorisation de la caméra au navigateur mobile/PC
        navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment" } })
        .then(function(stream) {
            function onScanSuccess(decodedText, decodedResult) {
                document.getElementById('result-bridge').value = decodedText;
            ...
            let html5QrcodeScanner = new Html5QrcodeScanner(
                ...
                { 
                    ...
                    qrbox: function(viewfinderWidth, viewfinderHeight) {
                        return { width: Math.floor(viewfinderWidth * 0.8), height: Math.floor(viewfinderHeight * 0.4) };
                    },
        ...
        .catch(function(err) {
            ...
            document.getElementById('interactive-reader').innerHTML = "<p style='color:white; padding:20px;'>⚠️ Veuillez autoriser l'accès à la caméra dans les réglages de votre navigateur pour utiliser le scanner.</p>";
        });
    </script>
    """
    
    # On affiche uniquement la caméra à l'écran
